      ...
      <section class="jumbotron text-center">
        <div class="container">
            <?php
                ...
                $query = "select * from `request1` where confirm is NULL;";
                if(count(fetchAll($query))>0){
                    foreach(fetchAll($query) as $row){
                        ?>
                
                    <h1 class="jumbotron-heading"><?php echo $row['heading'] ?></h1>
                      <p class="lead text-muted"><?php echo $row['describe'] ?></p>
					  <p class="lead text-muted"> <img src="dataimg/<?php echo $row['image']; ?>" style="max-width:100px;"  />  
                      <p>
